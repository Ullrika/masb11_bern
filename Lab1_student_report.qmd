...
If $Y=$ calcium level, we are looking for $P(Y>30)$, which is equivalent to $P(\log(Y) > \log(30))$. If $Y$ follows a lognormal distribution, then $\log(Y)$ follows a normal distribution. Set up a model for $\log(Y)$ and estimate its parameters. What distribution do the estimated parameters have?
...
Calculate the probability that \texttt{log(calcium level)} exceeds $\log(30)$. Provide the command you use to perform this calculation."
:::
